G.loader.ThreeLoader = Class.create(G.loader.Loader,{
    ...
    load: function(name, url){
        ...
        if(G.loaderCache.add(url,null)){
            this.loader.load(url, function(result){
                G.log("Load finished for " + name + ": " + url);
                G.loaderCache.update(url,result);
                G.resourceBank.add(name,result);
            });
        }
    ...
    hasBeenLoaded: function(url){
        return G.loaderCache.hasEntry(url);
    },
